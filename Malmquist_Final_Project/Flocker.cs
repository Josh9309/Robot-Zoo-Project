...
public class Flocker : FlockerVehicle {
    ...
    protected override void CalcSteeringForces()
    {
        ...
        for (int i = 0; i < fgm.Obstacles.Length; i++)
        {
            ultimateForce += AvoidObstacle(fgm.Obstacles[i], safeDistance) * avoidWeight;
        }
        ultimateForce += Cohesion(fgm.Centroid) * cohesionWeight;
        ultimateForce += Alignment(fgm.FlockDirection) * alignWeight;
